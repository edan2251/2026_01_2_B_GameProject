...
public class CardManager : MonoBehaviour
{
    ...
    public List<CardData> discardCards = new List<CardData>();  //버린 카드
    ...
    void Update()
    {
        ...
        ArrangeHand();      //손패 위치 업데이트 (드로우/버린 카드 후에도 부드럽게 이동)
    }
    ...
    {
        if (handCards.Count >= 6)    //손패 제한 6장 이상이면 드로우 하지 않음
        {
    ...
    public void DiscardCard(int handIndex)
    {
        ...
        for(int i = 0; i < cardObjects.Count; i++)      //카드 인덱스 설정
        {
            CardDisplay display = cardObjects[i].GetComponent<CardDisplay>();
